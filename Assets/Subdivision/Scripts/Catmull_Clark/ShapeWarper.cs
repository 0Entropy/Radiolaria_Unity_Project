...
namespace Subdivision.Core
{
  ...
  * complex geometries in this project become possible. */
        ...
        private Vector3 Horseshoe(Vector3 current)
        {
            ...
            return new Vector3((float)(radius * Math.Cos(2 * angle)), (float)(radius * Math.Sin(2 * angle)), current.z);
        }
        ...
        private Vector3 Swirl(Vector3 position)
        {
            ...
            return new Vector3((float)(radius * Math.Cos(inner)), position.y, (float)(radius * Math.Sin(inner)));
        }
    }
}