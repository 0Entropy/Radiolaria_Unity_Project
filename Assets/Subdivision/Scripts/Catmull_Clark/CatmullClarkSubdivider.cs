    ...
    public class CatmullClarkSubdivider : ISubdivider
    {
        ...
        private void CreateFacePoints(Shape shape)
        {
            ...
            foreach (Face face in shape.Faces)
            {
                ...
                face.FacePoint = points.AverageVector3();//new Point(Average(points));
        ...
        private Point CreateVertexPoint(List<Edge> allEdges, Point oldPoint)
        {
            ...
            //    the average of the centers of edges the point belongs to (avg_mid_edges),
            Vector3 avgMidEdges = oldPoint.Edges.Select(e => e.Middle).AverageVector3();// Vector3.Average(oldPoint.Edges.Select(e => e.Middle));
            ...
            Vector3 position = (float)m1 * oldPoint.Position + (float)m2 * avgFacePoints + (float)m3 * avgMidEdges;
        ...
        private Point CreateVertexPointForBorderPoint(Point oldPoint)
        {
            ...
            return new Point(positions.AverageVector3());//(Vector3.Average(positions));
        }
        ...
        {
            return points.Select(p => p.Position).AverageVector3();
            //return Vector3.Average(points.Select(p => p.Position));
        }

        private Vector3 Average(params Point[] points)
        {
            return points.Select(p => p.Position).AverageVector3();// Vector3.Average(points.Select(p => p.Position));
        }
    }
}